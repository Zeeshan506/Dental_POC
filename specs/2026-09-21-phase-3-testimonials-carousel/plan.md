...
- [ ] **Task 1.1**: Define structured placeholder review schema and records in `config/clinic.php`:
...
- [ ] **Task 1.2**: Define review helper or view data methods ensuring clean consumption across Blade components with null-safe fallbacks.

### Group 2: Variant A Expressive 2D Testimonials Carousel
- [ ] **Task 2.1**: Create `resources/views/components/variant-a/testimonials-carousel.blade.php`:
...
- [ ] **Task 2.2**: Implement restrained 2D stone navigation controls (previous/next buttons) with >= 44px touch targets and accessible `aria-label` attributes.

### Group 3: Variant B Calm Editorial Testimonials Carousel
- [ ] **Task 3.1**: Create `resources/views/components/variant-b/testimonials-carousel.blade.php`:
  ...
  - Line-clamped excerpt (`line-clamp-3`) ensuring locked card heights.
- [ ] **Task 3.2**: Implement understated editorial navigation controls (hairline arrows or text controls) with >= 44px touch targets.

### Group 4: Interactive Popover & Mobile Modal Engine
- [ ] **Task 4.1**: Create shared or modular review detail component / script (`resources/views/components/shared/review-modal.blade.php` or dedicated component script):
...
- [ ] **Task 4.2**: Verify Google Reviews source link opens in a new tab (`target="_blank" rel="noopener noreferrer"`).

### Group 5: View Orchestration & Regression Testing
- [ ] **Task 5.1**: Integrate `<x-variant-a.testimonials-carousel>` into `resources/views/variants/a/index.blade.php` and `<x-variant-b.testimonials-carousel>` into `resources/views/variants/b/index.blade.php`.
- [ ] **Task 5.2**: Create `tests/Feature/TestimonialsCarouselTest.php`:
...
- [ ] **Task 5.3**: Verify existing test suites (`tests/Feature/VariantATest.php`, `tests/Feature/VariantBTest.php`, `tests/Feature/VariantResolutionTest.php`) continue to pass without regressions.
