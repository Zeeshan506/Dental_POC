...
@endphp
<div
    class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 transition-transform duration-200 motion-reduce:transition-none"
    role="region"
...
>
    <nav class="flex items-center gap-1 p-1.5 bg-charcoal-900 border border-charcoal-700 rounded-full shadow-2xl" aria-label="Variant Navigation">
        ...
        <a
            href="{{ $urlVariantA }}"
            @class([
                'inline-flex items-center justify-center min-h-[44px] px-4 py-2 rounded-full text-xs font-medium transition-colors motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-stone-warm-200',
